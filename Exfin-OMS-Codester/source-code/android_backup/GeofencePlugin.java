...
public class GeofencePlugin extends Plugin {
    ...
    @PluginMethod
    public void getLastUnresolvedExit(PluginCall call) {
        try {
            Context context = getContext();
            boolean hasExit = OfficeGeofenceHelper.hasUnresolvedExit(context);
            JSObject ret = new JSObject();
            ret.put("hasUnresolvedExit", hasExit);
            if (hasExit) {
                ret.put("time", OfficeGeofenceHelper.getLastExitTime(context));
                ret.put("date", OfficeGeofenceHelper.getLastExitDate(context));
            }
            call.resolve(ret);
        } catch (Exception e) {
            call.reject("Failed to get unresolved exit: " + e.getMessage(), e);
        }
    }

    @PluginMethod
    public void clearUnresolvedExit(PluginCall call) {
        try {
            Context context = getContext();
            ...
            call.reject("Failed to clear unresolved exit: " + e.getMessage(), e);
        }
    }
