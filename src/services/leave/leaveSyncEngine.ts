...
export const syncPendingLeaves = async (): Promise<{ syncedCount: number; errorsCount: number }> => {
  ...
  if (!activeDb) {
    console.warn('Leave Sync Engine: Firestore getDb() instance unavailable.');
    return { syncedCount: 0, errorsCount: 0 };
  }
  ...
  for (const leave of pendingLeaves) {
    ...
    while (attempt < maxAttempts && !success) {
      ...
      try {
        ...
        const docRef = doc(activeDb, 'leaves', leave.id);
...
export const startLeaveSyncListeners = (options?: LeaveSyncScopeOptions): (() => void) => {
  ...
  let unsubAllowances: (() => void) | undefined;
  ...
  (async () => {
    try {
      ...
      if (options?.isAdminOrHR) {
        leavesQ = query(collection(activeDb, 'leaves'), limit(300));
      } else if (options?.isTeamLeader && options?.department) {
        leavesQ = query(collection(activeDb, 'leaves'), where('office', '==', options.department));
      } else if (options?.employeeCode) {
        leavesQ = query(collection(activeDb, 'leaves'), where('employeeCode', '==', options.employeeCode));
      } else {
        leavesQ = query(collection(activeDb, 'leaves'), limit(50));
      }
      ...
      unsubConfig = onSnapshot(
        doc(activeDb, 'system_settings', 'leave_settings'),
        (docSnap) => {
          if (docSnap.exists()) {
      ...
      if (options?.employeeCode && !options.isAdminOrHR) {
        allowancesQ = query(collection(activeDb, 'leave_balances'), where('employeeCode', '==', options.employeeCode));
      } else {
        allowancesQ = query(collection(activeDb, 'leave_balances'), limit(100));
      }

      unsubAllowances = onSnapshot(
        allowancesQ,
        (snapshot) => {
      ...
      console.error('Leave Sync Engine init error:', err);
    }
  })();
  ...
  return () => {
    ...
    if (unsubAllowances) unsubAllowances();
  };
};
