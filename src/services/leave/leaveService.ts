...
export const createLeaveRequest = async (
  ...
  if (navigator.onLine) {
    try {
      ...
      if (activeDb) {
        await setDoc(doc(activeDb, 'leaves', leaveId), leaveRecord);
...
export const cancelLeaveRequest = async (leaveId: string): Promise<LeaveRecord> => {
  ...
  if (navigator.onLine) {
    try {
      const activeDb = await getDb();
      if (activeDb) {
        await setDoc(doc(activeDb, 'leaves', leaveId), updatedLeave, { merge: true });
...
export const reviewLeaveRequest = async (
  ...
  if (!leave) {
    try {
      const activeDb = await getDb();
      if (activeDb) {
        const snap = await getDoc(doc(activeDb, 'leaves', leaveId));
        if (snap.exists()) {
...
export const updateLeaveConfig = async (config: LeaveConfig): Promise<void> => {
  ...
  if (navigator.onLine) {
    try {
      const activeDb = await getDb();
      if (activeDb) {
        await setDoc(doc(activeDb, 'system_settings', 'leave_settings'), config);
      }
    } catch (err) {
...
export const updateEmployeeAllowance = async (allowance: EmployeeAllowance): Promise<void> => {
  ...
  if (navigator.onLine) {
    try {
      const activeDb = await getDb();
      if (activeDb) {
        await setDoc(doc(activeDb, 'leave_balances', allowance.id), allowance);
