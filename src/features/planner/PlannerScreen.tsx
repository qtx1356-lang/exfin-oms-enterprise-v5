    ...
    <div className="flex flex-col gap-5 pb-16 text-white max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between pt-2 pb-2 border-b border-indigo-500/20">
        <div>
          ...
          <p className="text-xs text-indigo-200/80 font-medium mt-0.5">
            Operational Deliverables & Tasks for <span className="text-indigo-300 font-bold">{empDept}</span>
          </p>
        </div>
      ...
      <Card className="p-5 bg-gradient-to-br from-[#121B36] via-[#18244A] to-[#10172E] border border-indigo-500/30 shadow-xl rounded-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] font-black uppercase tracking-widest text-indigo-400">Sprint Delivery Rate</span>
          ...
          <div className="flex items-center gap-3 self-stretch sm:self-auto justify-between sm:justify-end">
            <div className="text-right">
              <p className="text-xs font-bold text-indigo-200">Completion Score</p>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
        <button
          ...
          className={`p-2.5 rounded-xl border flex flex-col items-center justify-between text-center transition-all ${
            activeCategory === 'today'
              ? 'bg-[#7C3AED] border-purple-400 text-white shadow-lg scale-[1.02]'
              : 'bg-[#211044] border-purple-500/20 text-purple-200 hover:bg-[#2D1B5A]'
          }`}
        >
          ...
          <span className="text-lg font-black mt-1 font-mono">{categoryCounts.today}</span>
        </button>
        ...
        >
          ...
          <span className="text-lg font-black mt-1 font-mono text-blue-300">{categoryCounts.upcoming}</span>
        </button>
        ...
        <button
          ...
          className={`p-2.5 rounded-xl border flex flex-col items-center justify-between text-center transition-all ${
            activeCategory === 'overdue'
              ? 'bg-rose-700 border-rose-400 text-white shadow-lg scale-[1.02]'
              : 'bg-[#211044] border-rose-500/30 text-rose-300 hover:bg-rose-950/40'
          }`}
        >
          <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-wider">
            <AlertTriangle className="w-3.5 h-3.5 text-rose-400" /> Overdue
          </div>
          <span className="text-lg font-black mt-1 font-mono text-rose-400">{categoryCounts.overdue}</span>
        </button>
        ...
        <button
          ...
          className={`p-2.5 rounded-xl border flex flex-col items-center justify-between text-center transition-all ${
            activeCategory === 'revision'
              ? 'bg-amber-600 border-amber-300 text-white shadow-lg scale-[1.02]'
              : 'bg-[#211044] border-amber-500/30 text-amber-300 hover:bg-amber-950/40'
          }`}
        >
          <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-wider">
            <RotateCcw className="w-3.5 h-3.5 text-amber-400" /> Revisions
          </div>
          <span className="text-lg font-black mt-1 font-mono text-amber-300">{categoryCounts.revision}</span>
        </button>
        ...
        <button
          ...
          className={`p-2.5 rounded-xl border flex flex-col items-center justify-between text-center transition-all ${
            activeCategory === 'completed'
              ? 'bg-emerald-700 border-emerald-400 text-white shadow-lg scale-[1.02]'
              : 'bg-[#211044] border-emerald-500/30 text-emerald-300 hover:bg-emerald-950/40'
          }`}
        >
          <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-wider">
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /> Completed
          </div>
          <span className="text-lg font-black mt-1 font-mono text-emerald-400">{categoryCounts.completed}</span>
        </button>
        ...
        >
          ...
          <span className="text-lg font-black mt-1 font-mono">{categoryCounts.total}</span>
        </button>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 no-scrollbar">
            <span className="text-[10px] font-extrabold text-purple-300/80 uppercase tracking-wider flex items-center gap-1 flex-shrink-0">
              <Filter className="w-3 h-3 text-[#A78BFA]" /> Priority:
            </span>
      ...
      <div className="space-y-3">
        ...
        {categorizedTasks.length > 0 ? (
          categorizedTasks.map((task) => {
            ...
            return (
              <Card
                ...
                className={`p-4.5 bg-[#2D1B5A] border rounded-2xl shadow-md cursor-pointer transition-all space-y-3 group ${
              ...
              >
                ...
                <div className="flex justify-between items-start gap-2">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="font-bold text-base text-white group-hover:text-purple-200 transition-colors">
                        {task.title}
                      </h3>
                    ...
                    {/* Metadata Grid */}
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-purple-300/80 pt-0.5">
                      <span className="flex items-center gap-1 font-semibold text-purple-200">
                        <Calendar className="w-3 h-3 text-[#A78BFA]" /> Assigned: {assignedDateDisplay}
                      </span>

                      <span className="flex items-center gap-1 font-semibold text-amber-300">
                        <Clock className="w-3 h-3 text-amber-400" /> Due: {task.dueDate} {task.dueTime ? `@ ${task.dueTime}` : ''}
                      </span>

                      {task.expectedCompletionTime && (
                        <span className="flex items-center gap-1 font-semibold text-indigo-300">
                          <Clock className="w-3 h-3 text-indigo-400" /> Expected: {task.expectedCompletionTime}
                        </span>
                      )}

                      {task.completedAt && (
                        <span className="flex items-center gap-1 font-semibold text-emerald-300">
                          <Check className="w-3 h-3 text-emerald-400" /> Completed: {new Date(task.completedAt).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}
                        </span>
                      )}
                ...
                {/* Description */}
                <p className="text-xs text-purple-200/90 line-clamp-2 bg-[#211044]/60 p-2.5 rounded-xl border border-purple-500/10">
                  {task.description}
                </p>
                ...
                <div className="pt-2 border-t border-purple-500/10 flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2 text-[10px] text-purple-300/70">
                    <span className="flex items-center gap-1">
                      <User className="w-3 h-3 text-purple-400" /> By {task.createdByName || 'Admin'}
                    </span>
                    {(task.comments || []).length > 0 && (
                      <span className="flex items-center gap-1">
                        <MessageSquare className="w-3 h-3 text-indigo-400" /> {(task.comments || []).length} comments
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {effectiveStatus === 'Assigned' && (
                      <Button
                        ...
                        className="py-1 px-3 text-[11px] font-bold bg-blue-600 hover:bg-blue-500 text-white flex items-center gap-1 rounded-xl"
                      >
                    ...
                    {(effectiveStatus === 'In Progress' || effectiveStatus === 'Revision Requested' || effectiveStatus === 'Overdue') && (
                      <Button
                        ...
                        className="py-1 px-3 text-[11px] font-bold bg-[#7C3AED] hover:bg-[#6D28D9] text-white flex items-center gap-1 rounded-xl"
                      >
          ...
          <div className="py-12 bg-[#211044] rounded-2xl border border-dashed border-purple-500/20">
            <EmptyState
              icon={CheckSquare}
          ...
          return (
            <div className="space-y-4 text-xs">
              ...
              <div className="p-3 bg-[#211044] rounded-2xl border border-purple-500/30 space-y-2">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-[11px] text-purple-200">
                  <div>
                    <span className="text-purple-300/70 font-bold block">Due Date & Time</span>
                    <span className="font-semibold text-white">{selectedTask.dueDate} {selectedTask.dueTime || ''}</span>
                  </div>
                  <div>
                    <span className="text-purple-300/70 font-bold block">Assigned Date</span>
                  ...
                  {selectedTask.expectedCompletionTime && (
                    <div>
                      <span className="text-purple-300/70 font-bold block">Expected Completion</span>
                      <span className="font-semibold text-indigo-300">{selectedTask.expectedCompletionTime}</span>
                    </div>
                  )}
                </div>
              </div>
                ...
                <div className="p-3 bg-amber-950/40 border border-amber-500/50 rounded-2xl space-y-2">
                  ...
                  <div className="space-y-1 pt-1">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-xl border border-amber-500/30 bg-[#211044] text-white text-xs"
                    />
                  </div>
                </div>
              )}
              ...
              <div className="p-3 bg-[#211044] rounded-2xl border border-purple-500/30 space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-black text-[#A78BFA] bg-purple-500/20 px-2.5 py-0.5 rounded-full border border-purple-500/30 font-mono">
                    {completionInput}%
                  </span>
                </div>
                ...
                {isRevision ? (
                  ...
                  <>
                    <Button
                      ...
                      disabled={isUpdating}
                      variant="secondary"
                      className="flex-1 py-3 bg-[#2D1B5A] hover:bg-[#3D2575] text-white font-bold rounded-2xl"
                    >
                    ...
                    <Button
                      ...
                      disabled={isUpdating}
                      className="flex-1 py-3 bg-[#7C3AED] hover:bg-[#6D28D9] font-bold rounded-2xl flex items-center justify-center gap-1.5"
                    >
      ...
      >
        {viewingRevisionsTask && (
          <div className="space-y-4 text-xs">
            <div className="p-3 bg-[#211044] rounded-2xl border border-purple-500/30">
            ...
            <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
              {(viewingRevisionsTask.revisions || []).length > 0 ? (
                viewingRevisionsTask.revisions!.map((rev, idx) => (
                  <div key={idx} className="p-3 bg-[#2D1B5A] rounded-xl border border-amber-500/30 space-y-2">
                    <div className="flex justify-between items-center">
                    ...
                    <div className="p-2 bg-[#1A0B36] rounded-lg border border-purple-500/20 text-purple-100">
                ...
                <div className="p-3 bg-[#2D1B5A] rounded-xl border border-amber-500/30 space-y-1">
                  <span className="font-bold text-amber-300">Revision Note:</span>
                  <p className="text-purple-100">{viewingRevisionsTask.reviewRemark}</p>
                </div>
              ) : (
                <p className="text-center py-4 text-purple-300 italic">No revision history recorded for this task.</p>
              )}
            </div>

            <Button
              onClick={() => setViewingRevisionsTask(null)}
              className="w-full py-2.5 bg-[#7C3AED] hover:bg-[#6D28D9] font-bold rounded-xl"
            >
              Close
            </Button>
          </div>
