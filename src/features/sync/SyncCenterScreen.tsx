    ...
    <div className="space-y-6 text-[var(--text-primary)] max-w-4xl mx-auto pb-16 font-sans">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button
            onClick={() => navigate(-1)}
            className="p-2 rounded-xl bg-[var(--card-surface)] border border-[var(--border)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--card-bg)] transition-all cursor-pointer"
            aria-label="Go Back"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-4 rounded-2xl bg-[var(--card-surface)] border border-[var(--border)] flex flex-col justify-between shadow-md">
      ...
      {/* Primary Actions Bar */}
      <div className="flex flex-wrap items-center justify-between gap-3 bg-[var(--card-surface)] border border-[var(--border)] p-3.5 rounded-2xl shadow-md">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            onClick={refreshData}
            disabled={isSyncing}
            className="text-xs border-[var(--border)] bg-[var(--card-bg)] hover:bg-[var(--card-surface)] text-[var(--text-primary)]"
          >
      ...
      <div className="space-y-4">
        {MODULE_LIST.map((mod) => {
          ...
          return (
            <div
              key={mod}
              className="bg-[var(--card-surface)] border border-[var(--border)] rounded-2xl overflow-hidden shadow-lg"
            >
              <div
                onClick={() => toggleModule(mod)}
                className="p-4 bg-[var(--card-surface)] hover:bg-[var(--card-bg)] cursor-pointer flex items-center justify-between transition-colors border-b border-[var(--border)]"
              >
                <div className="flex items-center gap-2.5">
                  <span className="font-extrabold text-sm text-[var(--text-primary)]">{mod}</span>
                  {modRecords.length > 0 ? (
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-black bg-[var(--card-bg)] text-[var(--primary-light)] border border-[var(--border)]">
                ...
                <div className="p-4 space-y-3 bg-[var(--card-surface)]">
                  {modRecords.length === 0 ? (
                    ...
                    modRecords.map((rec) => (
                      <div
                        key={rec.id}
                        className="p-3.5 bg-[var(--card-bg)] border border-[var(--border)] rounded-xl space-y-2"
                      >
                        ...
                        <div className="pt-2 border-t border-[var(--border)] flex flex-wrap items-center justify-between gap-2 text-[10px] text-[var(--text-secondary)]">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="px-2.5 py-1 rounded-lg bg-[var(--card-surface)] hover:bg-[var(--card-bg)] border border-[var(--border)] text-[var(--text-primary)] font-bold transition-all flex items-center gap-1 cursor-pointer"
                            >
        ...
        >
          <div className="space-y-4 font-sans text-[var(--text-primary)]">
            ...
            <div className="space-y-1">
              ...
              <pre className="p-3 bg-[var(--card-bg)] border border-[var(--border)] rounded-xl text-[11px] font-mono text-[var(--primary-light)] overflow-x-auto max-h-60">
