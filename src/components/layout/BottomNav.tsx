...
export const BottomNav: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-4 left-3 right-3 max-w-md mx-auto h-16 bg-[#2D1B5A]/95 backdrop-blur-2xl border border-purple-500/30 rounded-full flex items-center justify-around px-2 z-40 shadow-[0_10px_35px_rgba(0,0,0,0.5)]">
      {navItems.map((item) => (
        <NavLink
          key={item.path}
          to={item.path}
        ...
        >
          {({ isActive }) => (
            <>
              <div className={`flex items-center justify-center w-9 h-7 rounded-full mb-0.5 transition-all duration-300 ${
