...
// We can use a regex to replace up to self.addEventListener('activate', ...)
sw = sw.replace(/self\.addEventListener\('install',[\s\S]+?(?=self\.addEventListener\('activate',)/, newInstall + '\n\n');
